...
import type { AiDevsResponse, FinalAnswerFile } from "./types.ts";
...
export const saveTmpAnswer = async (
  ...
  const content = JSON.stringify(
    { episodeId, task, computedAt: new Date().toISOString(), answer },
    null,
    2
  );
...
export const saveFinalAnswer = async (
  ...
  const flag =
    response.flag ?? response.message?.match(/\{FLG:[^}]+\}/)?.[0] ?? null;
